...
public class CameraPreviewActivity extends AppCompatActivity implements GPUImageFilterGroupBase.IGroupStateChanged{
    ...
    private void init(){
        ...
        startActivityForResult(new Intent(this, DecoderActivity.class), 1);
...
//        Toast.makeText(CameraPreviewActivity.this, code, Toast.LENGTH_LONG).show();
        for (int i = 0; i < HardCodeData.itemList.size(); i++) {
            HardCodeData.EffectItem x = HardCodeData.itemList.get(i);
            Toast.makeText(getApplicationContext(), x.name + ":" +  code, Toast.LENGTH_LONG).show();
            if (x.name.equalsIgnoreCase(code.trim())) {
                doUpdateFilter(x);
                Toast.makeText(getApplicationContext(), x.name, Toast.LENGTH_LONG).show();
            }
        }
    }
